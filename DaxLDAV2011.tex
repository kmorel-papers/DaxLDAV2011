...
purpose of these modifies is twofold.  First, they provide the ability to
...
invalid connections before the job is dispatch to the parallel platform.
...
\fix{This description is incorrect.  Please fix.}
...
worklet makes a \textC{daxGetArrayValue\asterisk} function call, it checks
...
\textC{daxSetArrayValue\asterisk} function calls don't necessarily result
...
\textC{daxGetArrayValue\asterisk} call that triggered the execution of the
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llrrr}
    ...
    \multicolumn{5}{l}{\fix{The following are out of date!}}\\
    \multicolumn{5}{l}{\fix{The CUDA runs were for}}\\
    \multicolumn{5}{l}{\fix{\quad Gradient $\rightarrow$ Sine $\rightarrow$ Square $\rightarrow$ Cosine}}\\
    \multicolumn{5}{l}{\fix{This is the wrong pipeline and non-comparable}}\\
