...
\lstdefinelanguage{Dax}{
  morekeywords={daxWork,daxArray,daxFloat,daxFloat3,daxConnectedComponent,uint},
  morekeywords={[2]daxGetArrayValue,daxGetArrayValue3,daxSetArrayValue,daxGetConnectedComponent,daxGetNumberOfElements,daxGetWorkForElement,daxGetCellDerivative,daxSetArrayValue3},
  morekeywords={[3]__worklet__,__positions__,__dep__,__and__,__ref__,__connections__}
}
\lstset{
...
Regardless of how a data set is lain out in memory, the Dax API defines
...
flavors.  Fields can be applied to points and then interplated across the
...
of typed cell objects with methods choosen through templates or
polymorphism.

Fields are accessed through handle classes named
\textC{dax::exec::Field\asterisk}.  These field handles work with the work
objects to retreive actual values from the field.  Dax provides the
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llrrr}
    ...
    %% \multicolumn{5}{l}{Elevation $\rightarrow$ Gradient $\rightarrow$ Point Data to Cell Data} \\
